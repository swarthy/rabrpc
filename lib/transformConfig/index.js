...
module.exports = function transformConfig (config) {
  ...
  if (config.pub) {
    toArray(config.pub).forEach(item => Transform.transformProducer(cfg, item, PubSub.parseExchange))
  }
  if (config.sub) {
    toArray(config.sub).forEach(item => Transform.transformConsumer(cfg, item, PubSub.parseExchange, PubSub.parseQueue))
  }
