...
const config = {
  connection: {},
  req: {serviceName: 'req-res-test-service', replyTimeout: 30000, autoDelete: true}, // exchange config
...
describe('integration req-res', () => {
  before(() => {
    rpc.respond('v1.req-res-test-service.mul', (payload, actions, messageType) => {
      expect(messageType).to.be.eql('v1.req-res-test-service.mul')
      expect(payload).to.have.all.keys('a', 'b')
      return Promise.delay(50).then(() => mul(payload.a, payload.b))
    })
    rpc.respond('v1.req-res-test-service.div', (payload, actions, messageType) => {
      expect(messageType).to.be.eql('v1.req-res-test-service.div')
      expect(payload).to.have.all.keys('a', 'b')
      return Promise.delay(50).then(() => div(payload.a, payload.b))
    })
    rpc.respond('v1.req-res-test-service.make.me.some.#', (payload, actions, messageType) => {
